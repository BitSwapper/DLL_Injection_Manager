...
namespace FormDllInjector.Injection
{
    public class InjectionOrchestrator
    {
        ...
        private readonly RecentDllManager _recentDllManager;
        private readonly UIManager _uiManager;
        private readonly Func<ProcessItem> _refreshProcessListAction;
        ...
        public async Task PerformInjectionAsync(ProcessItem selectedProcessItem, DllListItem selectedDllItem)
        {
            ...
            try
            {
                ...
                {
                    injectorExeName = "Injector64MiddleMan.exe"; // UPDATED
                }
                else if(bitness == ProcessUtils.ProcessBitness.Bit32)
                {
                    injectorExeName = "Injector32MiddleMan.exe"; // UPDATED
                }
                else
                {
                ...
                {
                    _processMonitor.TrackLoadedDll(targetProcess.Id, dllPath, (nint)injectionResult.ModuleHandle);
        ...
        private async Task<InjectionResult> InjectWithHelperAsync(string injectorExeName, int processId, string dllPath)
        {
            ...
            try
            {
                ...
                string error = await helperProcess.StandardError.ReadToEndAsync();
                ...
                await helperProcess.WaitForExitAsync(new System.Threading.CancellationTokenSource(10000).Token);
