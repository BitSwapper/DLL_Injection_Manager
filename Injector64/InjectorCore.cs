...
namespace Injector64.Core;

internal static class InjectorCore
{
    const uint StandardProcessAccess = NativeMethods.PROCESS_CREATE_THREAD | NativeMethods.PROCESS_QUERY_INFORMATION | NativeMethods.PROCESS_VM_OPERATION | NativeMethods.PROCESS_VM_WRITE | NativeMethods.PROCESS_VM_READ;
        ...
        try
        {
            ...
            if(waitResult == NativeMethods.WAIT_OBJECT_0)
            {
                if(NativeMethods.GetExitCodeThread(threadHandle, out nint moduleHandleExitCode))
                {
                    ...
                    return new InjectionResult { Success = true, Message = StaticInjectorStrings.SuccessMsgDllInjected, ModuleHandle = (long)moduleHandleExitCode }; // Cast nint to long
                }
    ...
    static nint GetLoadLibraryAddress()
    {
        nint kernel32Handle = NativeMethods.GetModuleHandle(StaticInjectorStrings.Kernel32DllName);
        if(kernel32Handle == nint.Zero)
        {
            System.Diagnostics.Debug.WriteLine(string.Format(StaticInjectorStrings.ErrorMsgGetKernel32HandleFailed, Marshal.GetLastWin32Error()));
            return nint.Zero;
        }
        ...
        if(procAddress == nint.Zero)
            System.Diagnostics.Debug.WriteLine(string.Format(StaticInjectorStrings.ErrorMsgGetLoadLibraryAddressFailed, Marshal.GetLastWin32Error()));
